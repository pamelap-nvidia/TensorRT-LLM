...
class TestMoeAlltoAllSingleGPU(unittest.TestCase):
    ...
    @parameterized.expand([
        ...
        ("single_gpu", 2, 1, 8,
         torch.float16),  # some ranks have no data to send/recv
        ("single_gpu", 4, 5, 8,
         torch.float16),  # small input with larger world size
        ("single_gpu", 4, 901, 32768,
         torch.bfloat16),  # large input that reuses workspace
    ...
    def test_moe_alltoall_multi_rank(self, single_or_multi, world_size,
                                     input_entry_per_rank, vector_dim, dtype):
        ...
        if single_gpu:
            ...
            global_world_size = comm.Get_size()
        ...
        if single_gpu:
            ...
            for rank in range(world_size):
                with torch.cuda.stream(cuda_streams_all_ranks[rank]):
                    torch.ops.trtllm.moe_comm(input_tensors_all_ranks[rank],
            ...
            for rank in range(world_size):
                self.assertTrue(
                    torch.equal(output_tensors_all_ranks[rank],
                                ref_output_tensors_all_ranks[rank]))
        else:
            ...
            tllm.MnnvlMemory.comm = tllm.mpi_comm().Split(
                int(global_rank >= world_size), global_rank)
            if global_rank < world_size:
                rank = global_rank
                torch.cuda.set_device(global_rank)
                mapping = tllm.Mapping(world_size=world_size,
                                       rank=rank,
                                       tp_size=world_size,
                                       moe_ep_size=world_size)
                moe_workspace = tllm.MnnvlMemory(mapping, workspace_size)
                all_workspaces = moe_workspace.as_torch_strided_tensor(
                    torch.uint64)
